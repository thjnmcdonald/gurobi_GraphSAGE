...
def make_model_optimize_and_save(bt_procedures=[bt_lrr_conv],
    ...
    m.write(f'GraphSAGE_lps/{file_name}_{path}.lp')
    ...
    m.write(f'GraphSAGE_solutions/{file_name}_{path}.sol')

    with open(f'GraphSAGE_results.csv', mode = 'a+', newline='') as result_file:
        wr = csv.writer(result_file, quoting=csv.QUOTE_ALL)
        wr.writerow([path, gnn, layers, neurons, find_mol_of_length, m.Runtime, m.MIPGap, val, m.NumVars, m.NumConstrs, m.NumQConstrs, 'extra_run'])
