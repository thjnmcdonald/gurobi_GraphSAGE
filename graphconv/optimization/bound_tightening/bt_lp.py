...
from graphconv.optimization.bound_tightening.bt_utils import extract_bounds, build_bt_milp
...
def fbbt_linear(x_lb, x_ub, w, w_root, d_max):
    """
    ...
    max_norm_term = (d_max)/(((2)**(0.5)) * ((d_max)**(0.5)))
    
    m = gpy.Model()
    m.setParam('OutputFlag', False)  # Disable terminal output to reduce spam
    m.update()

    # TODO: test if you use only x_1 you get the same results as with x_1 and x_2. 
    
    x_1 = [m.addVar(lb=x_lb[i], ub=x_ub[i], vtype=gpy.GRB.CONTINUOUS) for i in range(nx)]
    x_2 = [m.addVar(lb=x_lb[i], ub=x_ub[i], vtype=gpy.GRB.CONTINUOUS) for i in range(nx)]
    y = m.addVar(lb=-gpy.GRB.INFINITY, vtype=gpy.GRB.CONTINUOUS)
    m.addConstr(gpy.quicksum(max_norm_term * w[i] * x_1[i] for i in range(len(x_1))) +
                gpy.quicksum(w_root[i] * x_2[i] for i in range(len(x_2))), gpy.GRB.EQUAL, y)
...
def bt_lrr(weights, bounds, d_max):
    ...
    for i, weights_i in enumerate(weights):
        ...
        for j in range(nu):
            x_lb, x_ub = fbbt_linear(x_prev_lb, x_prev_ub, w[:, j], w_root[:, j], d_max)
